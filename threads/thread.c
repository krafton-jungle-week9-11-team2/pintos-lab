...
void thread_init(void)
{
	...
	list_init(&all_list);		// project_2에서 추가
...
init_thread(struct thread *t, const char *name, int priority)
{
	...
	list_init(&(t->child_list)); // 이 코드 프로젝트2 새로 추가한 것
	sema_init(&t->load_sema, 0); // 이것도 새로 추가함

	sema_init(&t->exit_sema, 0); // wait에서 추가
	sema_init(&t->wait_sema, 0); // wait에서 추가

	/* ✅ 모든 스레드를 all_list에 기록 */
